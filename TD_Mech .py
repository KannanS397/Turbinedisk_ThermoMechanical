...
for i in range(len(P15)):
    ...
    mdb.models['Model-%s'%(i+1)].materials['Material-1'].Elastic(table=((200000.0, 0.3), 
        ))
    ...
    mdb.models['Model-%s'%(i+1)].StaticStep(name='Step-1', previous='Initial')
    ...
    mdb.models['Model-%s'%(i+1)].rootAssembly.Set(name='Set-2', vertices=
        ...
        ('[#800 ]', ), ))
    mdb.models['Model-%s'%(i+1)].ConcentratedForce(cf2=-600.0, createStepName='Step-1', 
    ...
    mdb.models['Model-%s'%(i+1)].RotationalBodyForce(centrifugal=ON, createStepName=
        'Step-1', magnitude=350.0, name='Load-2', point1=(0.0, 0.0, 0.0), point2=(
    ...
    mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF, 
        ...
        multiprocessingMode=DEFAULT, name='dispmodel-%s'%(i+1), nodalOutputPrecision=SINGLE, 
        numCpus=1, numGPUs=0, queue=None, resultsFormat=ODB, scratch='', type=
        ANALYSIS, userSubroutine='', waitHours=0, waitMinutes=0)
    mdb.jobs['dispmodel-'+str(i+1)].submit(consistencyChecking=OFF)
    mdb.jobs['dispmodel-'+str(i+1)].waitForCompletion()
    ...
    # a new odb file
    odb = openOdb(path='C:/Kannan/Suja/dispmodel-'+str(i+1)+'.odb')
...
workbook.close()
    #####################################################################################
 # Save by Admin on 2021_11_10-16.50.37; build 2018 2017_11_07-22.51.41 127140
    # Save by Admin on 2023_04_10-22.17.19; build 2018 2017_11_07-22.51.41 127140
