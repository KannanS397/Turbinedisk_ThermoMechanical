...
################################################################################################
    ...
    mdb.models['Model-%s'%(i+1)].materials['Material-1'].Elastic(table=((E[j], v[j]), 
        ))
    mdb.models['Model-%s'%(i+1)].materials['Material-1'].Conductivity(table=((k[j], ), 
        ))
    mdb.models['Model-%s'%(i+1)].materials['Material-1'].Expansion(table=((a[j], ), ))
    ...
    mdb.models['Model-%s'%(i+1)].HeatTransferStep(amplitude=RAMP, name='Step-1', previous=
    ...
    mdb.models['Model-%s'%(i+1)].FilmCondition(createStepName='Step-1', definition=
        EMBEDDED_COEFF, filmCoeff=Ht[j], filmCoeffAmplitude='', name='Int-1', 
        sinkAmplitude='', sinkDistributionType=UNIFORM, sinkFieldName='', 
        sinkTemperature=Tt[j], surface=
        mdb.models['Model-%s'%(i+1)].rootAssembly.surfaces['Surf-1'])
    mdb.models['Model-%s'%(i+1)].rootAssembly.Surface(name='Surf-2', side1Edges=
        mdb.models['Model-%s'%(i+1)].rootAssembly.instances['Part-1-1'].edges.getSequenceFromMask(
        ('[#ff00000f #f ]', ), ))
    mdb.models['Model-%s'%(i+1)].FilmCondition(createStepName='Step-1', definition=
        EMBEDDED_COEFF, filmCoeff=Hb[j], filmCoeffAmplitude='', name='Int-2', 
        sinkAmplitude='', sinkDistributionType=UNIFORM, sinkFieldName='', 
        sinkTemperature=Tb[j], surface=
    ...
    mdb.models['Model-%s'%(i+1)].parts['Part-1'].setElementType(elemTypes=(ElemType(
        elemCode=DCAX4, elemLibrary=STANDARD), ElemType(elemCode=DCAX3, 
    ...
    mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF, 
        ...
        multiprocessingMode=DEFAULT, name='TD_thermal-%s'%(i+1), nodalOutputPrecision=SINGLE, 
        numCpus=1, numGPUs=0, queue=None, resultsFormat=ODB, scratch='', type=
        ANALYSIS, userSubroutine='', waitHours=0, waitMinutes=0)
    mdb.jobs['TD_thermal-'+str(i+1)].submit(consistencyChecking=OFF)
    mdb.jobs['TD_thermal-'+str(i+1)].waitForCompletion()
            #####################################################################################
    #writing result
    import xlrd  
    ...
    # a new odb file
    odb = openOdb(path='C:/Kannan/Suja/TD_thermal-'+str(i+1)+'.odb')
