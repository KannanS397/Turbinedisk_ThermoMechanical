...
#############################
P16 = 21
workbook = xlsxwriter.Workbook('TD_thermal_fixed.xlsx')
...
################################################################################################
    ...
    mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF, 
        ...
        multiprocessingMode=DEFAULT, name='TD_Thermal_fixed-%s'%(i+1), nodalOutputPrecision=SINGLE, 
        numCpus=1, numGPUs=0, queue=None, resultsFormat=ODB, scratch='', type=
        ANALYSIS, userSubroutine='', waitHours=0, waitMinutes=0)
    mdb.jobs['TD_Thermal_fixed-'+str(i+1)].submit(consistencyChecking=OFF)
    mdb.jobs['TD_Thermal_fixed-'+str(i+1)].waitForCompletion()
            #####################################################################################
    #writing result
    import xlrd  
    ...
    # a new odb file
    odb = openOdb(path='C:/Kannan/Suja/TD_Thermal_fixed-'+str(i+1)+'.odb')
